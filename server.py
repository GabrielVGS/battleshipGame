...
class BattleshipServer:
    ...
    def handle_client(self, client_socket, player_id):
        ...
        with self.lock:
            ...
            if self.game.game_phase != 'setup' and len(self.clients) > 0:
                error_msg = {'type': 'error', 'message': 'O oponente desconectou. O jogo terminou.'}
        ...
        with self.lock:
            ...
            if self.game.game_phase == 'setup':
                ...
                if msg_type == 'placement_choice' and message['choice'] == 'auto':
                    ...
                    if self.game.is_valid_placement(player['board'], message['row'], message['col'], message['length'], message['direction']):
                        self.game.place_ship(player['board'], message['row'], message['col'], message['length'], message['direction'])
                        player['ships_to_place'].remove(message['length'])
                        
                        self.clients[player_id].send((json.dumps({
                            'type': 'placement_ok',
                            'board': player['board'],
                        ...
                        if not player['ships_to_place']:
                            player_ready = True
                            print(f"Servidor: {player_id} finalizou o posicionamento manual.")
                    else:
                        self.send_error(player_id, "Posicionamento inválido. Tente novamente.")
                ...
                if player_ready:
                    ...
                    if self.game.check_all_players_ready():
                        ...
                        print(f"Servidor: {player_id} está pronto. Aguardando o outro jogador.")

            elif self.game.game_phase == 'playing':
                if msg_type == 'shot':
                    success, result = self.game.make_shot(player_id, message['row'], message['col'])
                    if success:
                        self.broadcast_message({
                            'type': 'shot_result', 'result': result,
                            'row': message['row'], 'col': message['col'], 'shooter': player_id
                        })
                        self.send_game_state_to_all()
                    else:
    ...
    def start_game(self):
        ...
        print("Servidor: Ambos os jogadores estão prontos! Jogo iniciado. Vez do player_1.")
        self.broadcast_message({
            'type': 'game_start',
        ...
        for player_id in list(self.clients.keys()):
            if player_id in self.clients:
                ...
                if game_state:
                    ...
                    try:
                        self.clients[player_id].send((json.dumps(message) + '\n').encode())
                    except: pass
